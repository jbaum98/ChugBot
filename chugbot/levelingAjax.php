<?php
    session_start();
    require_once 'assignment.php'; // Includes functions and classes.
...
if (isset($_POST["save_changes"])) {
    ...
    foreach ($edah_ids as $edah_id) {
        getPrefListsForCampersByGroup(
            $edah_id, $block_id, $camperId2Name,
            $camperId2Edah, $edahId2Name, $camperId2Group2PrefList
        );
    }
...
if (isset($_POST["matches_and_prefs"])) {
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_NUM)) {
        ...
        $err = "";
        $result2 = $db->doQuery(
            "SELECT c.chug_id chug_id, c.name chug_name FROM chugim c, chug_instances i, edot_for_chug e " .
