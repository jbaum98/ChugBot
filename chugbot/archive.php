...
if ($_SERVER["REQUEST_METHOD"] == "GET" 
    ...
    if ($doArchive) {
        archiveCurrentDb(
            $dbErr, $preserveTables, $mysql, $mysqldump,
            $preserveTableId2Name, $thisYearArchive
        );
        if (empty($dbErr)) {
            $didArchiveDb = true;
...
EOM;
    ...
    $tableChooser->setGuideText(
        "Put a check next to the items you would like to carry over from $curCampYear to $nextCampYear. " .
