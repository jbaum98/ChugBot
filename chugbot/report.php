...
abstract class ReportTypes
{
    ...
    // menus.
    fillId2Name(
        $archiveYear, $chugId2Name, $dbErr,
        "chug_id", "chugim", "group_id",
        "groups"
    );
    fillId2Name(
        $archiveYear, $sessionId2Name, $dbErr,
        "session_id", "sessions"
    );
    fillId2Name(
        $archiveYear, $blockId2Name, $dbErr,
        "block_id", "blocks"
    );
    fillId2Name(
        $archiveYear, $groupId2Name, $dbErr,
        "group_id", "groups"
    );
    fillId2Name(
        $archiveYear, $edahId2Name, $dbErr,
        "edah_id", "edot"
    );
    fillId2Name(
        $archiveYear, $bunkId2Name, $dbErr,
        "bunk_id", "bunks"
    );
...
EOM;
    ...
    if ($doReport) {
        ...
        if ($reportMethod == ReportTypes::AllRegisteredCampers) {
            ...
            addWhereClause(
                $sql, $db, $activeEdahIds,
                "c.edah_id", true
            );
            addWhereClause(
                $sql, $db, $activeSessionIds,
                "c.session_id", true
            );
            addWhereClause(
                $sql, $db, $activeSessionIds,
            ...
            $haveWhere = addWhereClause($sql, $db, $activeBlockIds);
            addWhereClause(
                $sql, $db, $activeChugIds, "ch.chug_id",
                $haveWhere
            );
            ...
            addWhereClause(
                $inner, $db, $activeGroupIds,
                "c.group_id", true
            );
            addWhereClause(
                $inner, $db, $activeBlockIds,
                "b.block_id", true
            );
            addWhereClause(
                $inner, $db, $activeEdahIds,
                "ca.edah_id", true
            );
            $inner .= " GROUP BY 1";
            ...
            addWhereClause(
                $sql, $dbc, $activeBlockIds,
                "i.block_id", true
            );
            addWhereClause(
                $sql, $dbc, $activeEdahIds,
                "ca.edah_id", true
            );
            if ($sessionId) {
